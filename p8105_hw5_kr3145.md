...
sim_results_df |> 
  ...
  labs(
    title = " Group size vs Probability of Two People Sharing a Birthday",
    x = "Group Size",
    y = "Probability"
  ) 
```
...
The increase starts to plateau slightly as it gets closer to group size
of 50. At group size of 23, the probability that at least 2 people in
the group will share a birthday is 50%.

## Problem 2

Create function for running t-test for various means

``` r
t_test_fctn = function(m) {
  
  num_vec = rnorm(30, mean = m, sd = 5)
...
Run simulation 5000 times for mean 0 to 6, pulling estimate and p-value
...
Plot 1: Showing Effect Size and Power
...
Create dataframe with average estimate of sample mean for all samples
...
information of victims’ names, race, age, and sex, represented by
`victim_last`, `victim_first`, `victim_race`, `victim_age`, and
`victim_sex`, are included. `city`, `state`, `lat`, and `long` represent
the location of homicide. `disposition` represents how the homicide case
was concluded.
...
Summarize to obtain total homicides and unresolved homicides per city
...
Run prop.test for Baltimore, MD only
...
Pull estimate and confidence interval of the proportion of homicides
that are unsolved for Baltimore, MD only
...
Run prop.test to pull estimates and confidence intervals of the
proportion of homicides that are unsolved for all cities
...
<img src="p8105_hw5_kr3145_files/figure-gfm/unnamed-chunk-19-1.png" width="90%" />
